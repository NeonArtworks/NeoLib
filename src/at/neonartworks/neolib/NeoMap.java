...
public class NeoMap extends NeoPath{
	...
	public void setMap(double lat, double lon, int x, int y){
		...
		mapUrl = "https://maps.googleapis.com/maps/api/staticmap?maptype=satellite&center=" + st_lat + "," + st_lon + "&zoom=" + st_zoom +"&scale=3" +"&size=" + st_x + "x" + st_y+"&key="+this.apiKey;
	}
	...
	public void downloadGrayscaleMap(int x, int y, String URL){
		...
		try {
			URL mapURL = new URL(URL);
			int byteRead = 0, byteWritten = 0;
			//writeStream = new BufferedOutputStream(new FileOutputStream(path+".png"));
			File file = new File(path.getPath());
